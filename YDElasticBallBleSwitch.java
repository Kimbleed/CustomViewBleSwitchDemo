...
public class YDElasticBallBleSwitch extends View {
    ...
    private int scaleDuration = 1000;
    private int narrowDuration = 400;
    private int expandDuration = 5000;
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        mElasticBall.setmElasticBallInterface(new ElasticBall.ElasticBallInterface() {
            @Override
            public void onChange(Path path) {
                mElasticBallPath = new Path(path);
                postInvalidate();
            }
    ...
    private void drawElasticBall(Canvas canvas) {
        ...
        mPaint.setColor(mElasticBallColor);
        canvas.drawPath(mElasticBall.drawElasticCircle(), mPaint);
    }
    ...
    /**
     * mElasticBall.radius 扩展
     * @return
     */
    private ValueAnimator getExpandAnim() {
        ValueAnimator anim = ValueAnimator.ofFloat(mElasticBall.radius, mWidth,mElasticBall.radius);
