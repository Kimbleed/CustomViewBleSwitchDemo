...
public class ElasticBall extends Ball {
     ...
     * 弹性比例
     */
    private float mElasticPercent = 0.3f;
    /**
     * 位移距离
     */
    ...
    public void setElasticPercent(float elasticPercent) {
        mElasticPercent = elasticPercent;
    }
    /**
     * 对外公布方法，设置动画时间
     * @param duration
    ...
    /**
     * 设置接口
     * @param mElasticBallInterface
     */
    public void setmElasticBallInterface(ElasticBallInterface mElasticBallInterface) {
        this.mElasticBallInterface = mElasticBallInterface;
    }
    ...
    public void setmAnimPercent(float p) {
        this.mAnimPercent = p;
    ...
    public void startElasticAnim(PointF endPoint, ElasticBallInterface elasticBallInterface) {
        ...
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                Log.i("ElasticBall","per:"+ mAnimPercent);
                Log.i("ElasticBall","x:"+ x+ "|| y:"+y);
                if(mAnimPercent>=0 && mAnimPercent <= 0.2){
                    animStatus1();
                }
                else if(mAnimPercent > 0.2 && mAnimPercent <= 0.5){
    ...
    private void animStatus1() {
        ...
        if (mDirection == DIRECTION_LEFT) {
            ...
            rightX = mStartPoint.rightX;
            topX = mStartPoint.topX;
            bottomX = mStartPoint.bottomX;
            x = mStartPoint.x;
        } else if (mDirection == DIRECTION_RIGHT) {
            rightX = mStartPoint.rightX + percent * mElasticDistance;
            leftX = mStartPoint.leftX;
            topX = mStartPoint.topX;
            bottomX = mStartPoint.bottomX;
            x = mStartPoint.x;
    ...
    private void animStatus3() {
        ...
        if (mDirection == DIRECTION_LEFT) {
            leftX = mStatusPoint2.leftX - Math.abs(percent * (mEndPoint.leftX - mStatusPoint2
                    .leftX));
    ...
    public Path drawElasticCircle(){
        return drawElasticCircle(topX, topY, offsetTop, offsetTop,
                bottomX, bottomY, offsetBottom, offsetBottom,
                leftX, leftY, offsetLeft, offsetLeft,
                rightX, rightY, offsetRight, offsetRight);
    }
    ...
    public void setXY(float x,float y){
        ...
        bottomY -= offY;
        rightY -=offY;
        refresh(x,y,topX,topY,bottomX,bottomY,leftX,leftY,rightX,rightY);
